...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_keyword'])) {
			foreach ($data['product_keyword'] as $key => $value) {
				if ($value) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'product_id=" . (int)$product_id . "', keyword = '" . $this->db->escape($value) . "', language_id = '" . (int)$key . "'");
				}
			}
		}
	...
	public function getProductKeywords($product_id) {
		...
		foreach ($query->rows as $result) {
			$product_keyword_data[$result['language_id']] = $result['keyword'];
		}
